...
> 미국 주식 시장에서 기업 내부자(CEO, CFO, 이사 등)의 **장내 직접 매수(Code P)** 및 **대량 장내 매도(Code S)** 공시(SEC Form 4)를 실시간 감지하여 전달하는 무서버 파이프라인.
...
3. **분할 거래 합산(Aggregation)**: 동일 공시 내 여러 날짜/가격으로 쪼개어 거래한 내역을 통합 합산하여 $100,000 이상 여부 판정.
