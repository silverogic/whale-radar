...
> 미국 주식 시장에서 기업 내부자(CEO, CFO, 이사)의 **장내 매수/매도(SEC Form 4)** 및 대형 기관투자자·행동주의 펀드(Whale)의 **5% 이상 대량 지분 보고(Schedule 13D/13G)**를 실시간 감지하여 전달하는 무서버 파이프라인.

---

## 📌 주요 특징
1. **노이즈 100% 제거**: 스톡옵션 행사(Code M), 무상 증여(Code A), 장외 거래 등 주가 시그널과 무관한 공시 필터링.
2. **진짜 매수(Code P) & 매도(Code S) 포착**:
   * 🟢 **장내 매수(BUY)**: 경영진의 자기 자본 직접 매수 시그널
   * 🔴 **장내 매도(SELL)**: 임원 대량 차익 실현 / 리스크 시그널
3. **대형 기관(Whale) 5%+ 지분 포착 (Schedule 13D / 13G)**:
   * 🟣 **Schedule 13D**: 경영참여 및 행동주의 펀드 지분 포착
   * 🔵 **Schedule 13G**: 패시브/기관투자자의 5%+ 대량 단순 지분 보유
4. **분할 거래 합산(Aggregation)**: 동일 공시 내 여러 날짜/가격으로 쪼개어 거래한 내역을 통합 합산 판정.
5. **SEC 규정 준수**: User-Agent 정책 및 초당 10회 미만 Rate Limiting(0.15초 대기) 준수.
